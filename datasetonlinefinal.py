...
class BilingualDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        dec_num_padding_tokens = self.seq_len - 1 - len(dec_input_tokens)
        ...
        decoder_input = torch.cat(
            [
                ...
                torch.tensor([self.tgt_pad_token.item()] * dec_num_padding_tokens, dtype=torch.int64)
            ],
            dim=0
        )
        ...
        decoder_padding_mask = ((decoder_input == self.tgt_pad_token).unsqueeze(0).unsqueeze(0) | look_ahead_mask(decoder_input.size(0))).int()  # (1, seq_len, seq_len)

        return {
            "encoder_input": encoder_input,  # (seq_len)
            "decoder_input": decoder_input,  # (seq_len)
            "encoder_mask": encoder_padding_mask,  # (1, 1, seq_len)
            "decoder_mask": decoder_padding_mask,  # (1, seq_len) & (1, seq_len, seq_len),
            "label": label,  # (seq_len)
            "src_text": src_text,
            "tgt_text": tgt_text
        }
